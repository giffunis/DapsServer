...
exports.show = function(req,res){
  ...
  console.log(id);
  activityDB.activity.find({ patientId: mongojs.ObjectId(id), created_at: { '$gte': desde, '$lte': hasta}}, function (err, docs) {
    if(err){
      ...
      for (var i = 0; i < docs.length; i++) {
        punto = [docs[i].created_at.getMonth() + "/" + docs[i].created_at.getDay(), docs[i].steps];
        data.push(punto);
      }
